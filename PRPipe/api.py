...
class PRN:
    ...
    def dlib_detect(self, image):
        if self.face_detector is None:
            raise ValueError("No face detector initialized.")
    ...
    def get_Transposed_landmarks(self, pos):
        '''
        Args:
            pos: the 3D position map. shape = (256, 256, 3).
        Returns:
            kpt: 68 3D landmarks. shape = (3, 68).
        '''
        kpt = pos[self.uv_kpt_ind[1,:], self.uv_kpt_ind[0,:], :]
        kpt = np.transpose(kpt) # or kpt = kpt.T
        return kpt
